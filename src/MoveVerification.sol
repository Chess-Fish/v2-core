 ...
 * @dev Forked from: https://github.com/marioevz/chess.eth (Updated from
 * Solidity 0.7.6 to 0.8.17 & Added features and functionality)
 *
 * @notice This contract handles the logic for verifying the validity moves on
 * the chessboard. Currently, pawns autoqueen by default.
 */
contract MoveVerification {
    ...
    /**
     * @dev For castling masks, mask only the last bit of an uint8, to block any
     * under/overflows.
     */
    ...
    /**
     * @dev Calculates the outcome of a game depending on the moves from a
     * starting position.
     ...
     *     @param startingTurnBlack Whether the starting player is the black
     * pieces
     *     @param moves is the input array containing all the moves in the game
     *     @return outcome can be 0 for inconclusive, 1 for draw, 2 for white
     * winning, 3 for black winning
     */
    function checkGame(
        ...
        if (moves[moves.length - 1] == accept_draw_const) {
            ...
            // Assumes that signatures have been checked and moves are in
            // correct order
            outcome = ((moves.length % 2) == 1) != currentTurnBlack ? black_win_outcome : white_win_outcome;
        } else {
            // Check entire game
            for (uint256 i = 0; i < moves.length;) {
                (gameState, opponentState, playerState) =
                    verifyExecuteMove(gameState, moves[i], playerState, opponentState, currentTurnBlack);
                unchecked {
                    i++;
                }

                require(!checkForCheck(gameState, opponentState), "inv check");
                //require (outcome == 0 || i == (moves.length - 1), "Excesive
                // moves");
    ...
    /**
     * @dev Calculates the outcome of a single move given the current game
     * state.
     *          Reverts for invalid movement.
     *     @param gameState current game state on which to perform the movement.
     *     @param move is the move to execute: 16-bit var, high word = from pos,
     * low word = to pos
    ...
    function verifyExecuteMove(
        ...
        if (fromType == pawn_const) {
            (newGameState, newPlayerState) = verifyExecutePawnMove(
                gameState, fromPos, toPos, (uint8)(move >> 12), currentTurnBlack, playerState, opponentState
            );
            ...
            (newGameState, newPlayerState) =
                verifyExecuteKingMove(gameState, fromPos, toPos, currentTurnBlack, playerState);
        } else {
    ...
    /**
     * @dev Calculates the outcome of a single move of a pawn given the current
     * game state.
    ...
    function verifyExecutePawnMove(
        ...
        newPlayerState = playerState;
        // require ((currentTurnBlack && (toPos < fromPos)) ||
        // (!currentTurnBlack && (fromPos < toPos)), "inv move");
        ...
        if (diff == 8 || diff == 16) {
            ...
            if ((uint8)(opponentState & en_passant_const) != toPos) {
                if (
                    (pieceToPosition == 0) || (currentTurnBlack == ((pieceToPosition & color_const) == color_const)) // Must
        ...
        if ((currentTurnBlack && ((toPos >> 3) == 0x0)) || (!currentTurnBlack && ((toPos >> 3) == 0x7))) {
            ...
            ||
            (moveExtra == rook_const) || (moveExtra == queen_const), "inv
            prom");
            */
            // auto queen promote
            ...
            newGameState = setPosition(
                zeroPosition(newGameState, toPos), toPos, currentTurnBlack ? moveExtra | color_const : moveExtra
            );
        }
    ...
    /**
     * @dev Calculates the outcome of a single move of a knight given the
     * current game state.
     *          Returns invalid_move_constant for invalid movement.
     *     @param gameState current game state on which to perform the movement.
     *     @param fromPos is position moving from.
    ...
    /**
     * @dev Calculates the outcome of a single move of a bishop given the
     * current game state.
     *          Returns invalid_move_constant for invalid movement.
     *     @param gameState current game state on which to perform the movement.
     *     @param fromPos is position moving from.
    ...
    /**
     * @dev Calculates the outcome of a single move of a rook given the current
     * game state.
     *          Returns invalid_move_constant for invalid movement.
     *     @param gameState current game state on which to perform the movement.
     *     @param fromPos is position moving from.
    ...
    /**
     * @dev Calculates the outcome of a single move of the queen given the
     * current game state.
     *          Returns invalid_move_constant for invalid movement.
     *     @param gameState current game state on which to perform the movement.
     *     @param fromPos is position moving from.
    ...
    /**
     * @dev Calculates the outcome of a single move of the king given the
     * current game state.
     *          Returns invalid_move_constant for invalid movement.
     *     @param gameState current game state on which to perform the movement.
     *     @param fromPos is position moving from. Behavior is undefined for
     * values >= 0x40.
     *     @param toPos is position moving to. Behavior is undefined for values
     * >= 0x40.
     *     @param currentTurnBlack true if it's black turn
     *     @return newGameState the new game state after it's executed.
     */
    function verifyExecuteKingMove(
        ...
        if ((h <= 1) && (v <= 1)) {
            ...
            if (!pieceUnderAttack(gameState, fromPos)) {
                ...
                // Reasoning: castilngRookPosition resolves to an invalid toPos
                // when the rook or the king have already moved.
                ...
                if (castilngRookPosition + 2 == toPos) {
                    ...
                    if ((getInBetweenMask(castilngRookPosition, fromPos) & gameState) == 0) {
                        ...
                        if (!pieceUnderAttack(newGameState, fromPos - 1)) {
                            return (
                                commitMove(
                                    commitMove(newGameState, fromPos - 1, toPos), castilngRookPosition, fromPos - 1
                                    ),
                                newPlayerState
                            );
                        }
     ...
     *     @param gameState The current game state on which to perform the
     * movement.
     *     @param fromPos The position from which the queen is moving.
     *     @param playerState The player's state containing information about
     * the king position.
        ...
        returns (bool)
    {
        ...
        uint8 toPos;
        uint8 kingPos = (uint8)(playerState >> king_pos_bit); /* Kings position
            cannot be affected by Queen's movement */
     ...
     *     @param gameState The current game state on which to perform the
     * movement.
     *     @param fromPos The position from which the bishop is moving. Behavior
     * is undefined for values >= 0x40.
     *     @param playerState The player's state containing information about
     * the king position.
     *     @param currentTurnBlack True if it's black's turn, false otherwise.
     *     @return A boolean indicating whether the move is valid or not.
        ...
        returns (bool)
    {
        ...
        uint8 toPos;
        uint8 kingPos = (uint8)(playerState >> king_pos_bit); /* Kings position
            cannot be affected by Bishop's movement */
     ...
     *     @param gameState The current game state on which to perform the
     * movement.
     *     @param fromPos The position from which the rook is moving. Behavior
     * is undefined for values >= 0x40.
     *     @param playerState The player's state containing information about
     * the king position.
     *     @param currentTurnBlack True if it's black's turn, false otherwise.
        ...
        returns (bool)
    {
        ...
        uint8 toPos;
        uint8 kingPos = (uint8)(playerState >> king_pos_bit); /* Kings position
            cannot be affected by Rook's movement */
     ...
     *     @param gameState The current game state on which to perform the
     * movement.
     *     @param fromPos The position from which the knight is moving. Behavior
     * is undefined for values >= 0x40.
     *     @param playerState The player's state containing information about
     * the king position.
     *     @param currentTurnBlack True if it's black's turn, false otherwise.
        ...
        returns (bool)
    {
        ...
        uint8 toPos;
        uint8 kingPos = (uint8)(playerState >> king_pos_bit); /* Kings position
            cannot be affected by knight's movement */
    ...
    function checkPawnValidMoves(
        ...
        uint256 newGameState;
        uint8 toPos;
        uint8 moveExtra = queen_const; /* Since this is supposed to be endgame,
            movement of promoted piece is irrelevant. */
        uint8 kingPos = (uint8)(playerState >> king_pos_bit); /* Kings position
            cannot be affected by pawn's movement */

        unchecked {
            toPos = currentTurnBlack ? fromPos - 7 : fromPos + 7;
            (newGameState,) = verifyExecutePawnMove(
                gameState, fromPos, toPos, moveExtra, currentTurnBlack, playerState, opponentState
            );
            if ((newGameState != invalid_move_constant) && (!pieceUnderAttack(newGameState, kingPos))) {
                return true;
            }
     ...
     *     @return returns true if any of the pieces in the current offest has
     * legal moves
     */
    ...
    /**
     * @dev Checks the endgame state and determines whether the last user is
     * checkmate'd or
     *          stalemate'd, or neither.
     *     @param gameState Game state from which start the movements
     *     @param playerState State of the player
     *     @return outcome can be 0 for inconclusive/only check, 1 stalemate, 2
     * checkmate
     */
    ...
    /**
     * @dev Calculates the horizontal movement between two positions on a
     * chessboard.
     *     @param fromPos The starting position from which the movement is
     * measured.
    ...
    /**
     * @dev Calculates the vertical movement between two positions on a
     * chessboard.
     *     @param fromPos The starting position from which the movement is
     * measured.
     *     @param toPos The ending position to which the movement is measured.
    ...
    /**
     * @dev Checks if the king in the given game state is under attack (check
     * condition).
     *     @param gameState The current game state to analyze.
     *     @param playerState The player's state containing information about
     * the king position.
     *     @return A boolean indicating whether the king is under attack (check)
     * or not.
     */
    ...
    /**
     * @dev Checks if a piece at the given position is under attack in the given
     * game state.
     * @param gameState The current game state to analyze.
    ...
    function pieceUnderAttack(uint256 gameState, uint8 pos) public pure returns (bool) {
        ...
        unchecked {
            ...
            currPos = pos + 17;
            if (
                (currPos < 0x40) && ((currPos & 0x7) > (pos & 0x7))
            ...
            currPos = pos - 17;
            if (
                (currPos < pos) && ((currPos & 0x7) < (pos & 0x7))
                    && (((uint8)(gameState >> (currPos * piece_bit_size)) & 0xf) == enemyKnight)
            ) return true;
    ...
    /**
     * @dev Commits a move into the game state. Validity of the move is not
     * checked.
     *     @param gameState current game state
    ...
    function commitMove(uint256 gameState, uint8 fromPos, uint8 toPos) public pure returns (uint256) {
        ...
        uint256 newGameState = gameState & ~(0xF << bitpos);
     ...
     *     @param gameState current game state
     *     @param pos is the position to zero out: 6-bit var, 3-bit word, high
     ...
     *          Behavior is undefined for position values greater than 0x3f
     *     @param gameState current game state
     *     @param pos is the position to set the piece: 6-bit var, 3-bit word,
     ...
     *          Behavior is undefined for position values greater than 0x3f
     *     @param gameState current game state
     *     @param pos is the position to get the piece: 6-bit var, 3-bit word,
     * high word = row, low word = column.
