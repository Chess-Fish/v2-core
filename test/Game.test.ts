...
describe("ChessFish Game Verification Unit Tests", function () {
	...
	describe("Functionality Tests", function () {
		...
		it("Should get move hex at position", async function () {
			...
			let move = await chessGame.convertToMove(fromPos, toPos);

			expect(move).to.equal(parseInt("0x2db", 16));

			// testing front end view function
			let convertedMove = await chessGame.convertFromMove(move);
		...
		it("Should convert move to hex move", async function () {
			...
			let hex_move = await chessGame.moveToHex("e2e4");
		...
		it("Should get outcome from checkEndgame using hex moves", async function () {
			const { moveVerification, initalState, initialWhite, initialBlack } = await loadFixture(
				deploy
			);
		...
		it("Should get outcome from checkEndgame of stalemate using algebraic chess notation", async function () {
			...
			for (let i = 0; i < moves.length; i++) {
				let hex_move: string = await chessGame.moveToHex(moves[i]);
				hex_moves.push(hex_move);
			}
		...
		it("Should get outcome from checkEndgame of stalemate using algebraic chess notation", async function () {
			...
			for (let i = 0; i < moves_stalemate.length; i++) {
				let hex_move: string = await chessGame.moveToHex(moves_stalemate[i]);
				hex_moves.push(hex_move);
			}
		...
		it("Should check move moveToHex functions", async function () {
			...
			for (let i = 0; i < moves_stalemate.length; i++) {
				let hex_moveSC = await chessGame.moveToHex(moves_stalemate[i]);
